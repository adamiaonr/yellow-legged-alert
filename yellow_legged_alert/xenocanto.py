...
class Recording:
    ...
    def _generate_filename(self) -> str:
        return (
            f"{self.genus}_{self.species}_{self.subspecies}_{self.id}.{self.audio_type}"
        ).lower()
    ...
    def from_dict(cls, data: dict) -> "Recording":
        try:
            ...
            raise KeyError('error while generating filename for recording') from exc
...
def _requests_adapter(url: str) -> dict:
    return requests.get(url, verify=True, timeout=10).json()


def get_recordings_list(
    genus: str, subspecies: str, api_adapter=_requests_adapter
) -> list[Recording]:
...
def download_recordings(
    ...
    wait: int = 1,
    force: bool = False,
    download_adapter=_wget_adapter,
) -> defaultdict[Path]:

    if wait < 1:
        raise ValueError(
            f"wait < 1 seconds not allowed : would overload {XENO_CANTO_NAME} API"
        )
