...
def split_audio_on_silence(waveform:np.ndarray, sample_rate:int, 
  limits:list = [], remove_noise:bool = True) -> tuple[list[np.ndarray], int]:
  # (optional) reduce background noise of audio clip beforhand to make splitting easier
  intervals = lr.effects.split(y = reduce_noise(waveform, sample_rate)[0] if remove_noise else waveform)
  waveforms = [ waveform[i[0]:i[1]] for i in intervals
      if not limits 
      or (limits[0] <= ((i[1] - i[0]) / sample_rate) <= limits[1]) 
    ]
  
  return waveforms, sample_rate

def split_audio_on_time(waveform:np.ndarray, sample_rate:int, 
  duration:int = 1, trim:bool = False) -> tuple[list[np.ndarray], int]:
